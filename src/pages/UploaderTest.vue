...
q-layout
  ...
  q-uploader(url='http://localhost:4444/upload', label='Individual upload', multiple, style='max-width: 300px')
  q-uploader(
...
export default {
  ...
  methods: {
    ...
    test2() {
      this.info = navigator.platform
    },
    async getInfo() {
      const s = await navigator.mediaDevices.enumerateDevices()
      console.log({ s })
      this.info = JSON.stringify(s, null, 2)
